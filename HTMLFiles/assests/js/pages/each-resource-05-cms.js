$(document).ready(function () {
    ...
    $("#res5_title").on('keyup',function(event) {
      var stt = $(this).val();
      $("#res5_meta_title").val(stt);
    });
   
  
    //Add Content Available Langauge
    $(document)
  .on("click", ".res5_lang_container .add_lgn_btn", function (e) {
    e.preventDefault();
    // alert('hi');
      var res5langinput_value = $(".res5_lang_container .input_value");
      var is_res5langinput_value_empty = false;
      for (var i = 0; i < res5langinput_value.length; i++) {
        if($(res5langinput_value[i]).val() === ""){
          is_res5langinput_value_empty = true;
        }
       
      }
      if(is_res5langinput_value_empty === false){
    var blang_container_html =
     ...
     <div class="col-sm-2">
      ...
      $(".res5_lang_url_container").prepend(blang_container_html);
  ...
  $(document)
  .on("click", ".res5_lang_url_container .add_lgn_btn", function (e) {
    e.preventDefault();
  ...
  //Add Content Available Langauge
    ...
    //add gallery
    $(document)
  .on("click", ".res5_gallery_container  .member-add", function (e) {
    e.preventDefault();
      var res5galleryinput_value = $(".res5_gallery_container .input_value");
      var is_res5galleryinput_value_empty = false;
      for (var i = 0; i < res5galleryinput_value.length; i++) {
        if($(res5galleryinput_value[i]).val() === ""){
          is_res5galleryinput_value_empty = true;
        }
       
      }
      if(is_res5galleryinput_value_empty === false){
    var res5galleryinput_html =
    ...
    <div class="col-sm-10 mb-3">
      ...
      $(".res5_each_gallery_container").prepend(res5galleryinput_html);
  ...
  .on("click", ".res5_gallery_container .each-form-set .member-del", function (e) {
    e.preventDefault();
 ...
 //check_type
$(document)
.on("change", ".res5_help_container .check_type", function (e) {
...
$(document)
.on("click", ".res5_help_container .member-add", function (e) {
  e.preventDefault();
  // alert('hi');
    var res5_helpinput_value = $(".res5_help_container .input_value");
    var is_res5_helpinput_value_empty = false;
    for (var i = 0; i < res5_helpinput_value.length; i++) {
      if($(res5_helpinput_value[i]).val() === ""){
        is_res5_helpinput_value_empty = true;
      }
     
    }
    if(is_res5_helpinput_value_empty === false){
  var res5help_container_html =
  `<div class="row each-form-set mb-3">
  ...
  <div class="var_html">
    ...
    $(".res5_each_help_container").prepend(res5help_container_html);
...
$(document)
.on("click", ".res5_each_help_container .member-add", function (e) {
  e.preventDefault();

  
})
.on("click", ".res5_each_help_container .each-form-set .member-del", function (e) {
  e.preventDefault();
  $(this).closest(".each-form-set").remove();
  return false;
});
...
//Help Container End
 ...
 //Form Validation Start
    function checkvalidation() {
      ...
      //Basic Details
      var res5_title = $("#res5_title").val();
      if (res5_title === "") {
        info += "Please enter title<br>";
        isperfect = false;
      }
      var res_url = $("#res5_slug").val();
      if (res_url === "") {
        info += "Please enter URL Slug<br>";
        isperfect = false;
      }
      var res5_excerpt = $("#res5_excerpt").val();
      if (res5_excerpt === "") {
        info += "Please fill the Excerpt with max 360 words<br>";
        isperfect = false;
      }

      var resImage = $("#res5_thumbimg").val();
      // Allowing file type
    ...
    //Meta1 Details
      var res5_meta_title = $("#res5_meta_title").val();
      if (res5_meta_title === "") {
        info += "Please enter the Meta title<br>";
        isperfect = false;
      }
      var res5_meta_excerpt = $("#res5_meta_excerpt").val();
      if (res5_meta_excerpt === "") {
        info += "Please enter the Meta description<br>";
        isperfect = false;
      }
      var fileInput = $("#res5_meta_image").val();
      // Allowing file type
      ...
      var borrow_request = $("#borrow_request").val();
      if (borrow_request === "") {
        info += "Please enter Rais Borrow Request <br>";
        isperfect = false;
      }

      var res5_zip = $("#res5_zip").val();
      // Allowing file type
      var allowedExtensions3 = /(\.zip)$/i;
      if (res5_zip === "") {
      info += "Please upload a file for download<br>";
      isperfect = false;
      } else if (!allowedExtensions3.exec(res5_zip)) {
      info += "Please upload zip file for download<br>";
      isperfect = false;
      }
     
      //Content Avaliable
      var rev_lang_input_value = $(".res5_lang_container .input_value");
   ...
   //Help Material
    var helpinput_value = $(".res5_help_container .input_value");
    ...
    var res5help_file_uploader = $(".res5_help_container .input_value.file_uploader");
    var is_res5help_file_uploader_empty = false;
    let allowedExtensions4 =
        /(\.png|\jpe?g|\.odt|\.pdf|\.tex|\.txt|\.rtf|\.wps|\.docx|\.doc)$/i;
        
    for (var i = 0; i < res5help_file_uploader.length; i++) {
      let each_res5help_file_uploader = $(res5help_file_uploader[i]).val();
      //alert(each_res5help_file_uploader);
      if(each_res5help_file_uploader === ""){
        is_res5help_file_uploader_empty = true;

        info += "Please upload a file in Help Section<br>";
        isperfect = false;
      }else if (!allowedExtensions4.exec(each_res5help_file_uploader)) {
    ...
    //Gallery Add
    var galleryinput_value = $(".res5_gallery_container .input_value");
    ...
    var galleryinput_file_uploader = $(".res5_gallery_container .input_value.file_uploader");
    var is_galleryinput_file_uploader_empty = false;
    let allowedExtensions5 =
        /(\.png|\jpe?g|\.odt|\.pdf|\.tex|\.txt|\.rtf|\.wps|\.docx|\.doc)$/i;
        
    for (var i = 0; i < galleryinput_file_uploader.length; i++) {
      let each_res5_file_uploader = $(galleryinput_file_uploader[i]).val();
      //alert(each_res5help_file_uploader);
      if(each_res5_file_uploader === ""){
        is_galleryinput_file_uploader_empty = true;

        info += "Please upload a file in Gallery<br>";
        isperfect = false;
      }else if (!allowedExtensions5.exec(each_res5_file_uploader)) {
        info += "Please upload a valid file Extension in Help Section<br>";
        isperfect = false;
    ...
    var res5subject = document.getElementById("res5subject");
    if (res5subject.selectedIndex <=0) {
      info += "Please Select Subject <br>";
      isperfect = false;
    } 
  
    var res5language = document.getElementById("res5language");
    if (res5language.selectedIndex <=0) {
      info += "Please Select Language <br>";
      isperfect = false;
    } 

    var res5topics = document.getElementById("res5topics");
    if (res5topics.selectedIndex <=0) {
      info += "Please Select Topics <br>";
      isperfect = false;
    }
    var res5eprojects = document.getElementById("res5projects");
    if (res5eprojects.selectedIndex <=0) {
      info += "Please Select Projects <br>";
      isperfect = false;
    }
    var res5avail = document.getElementById("res5avail");
    if (res5avail.selectedIndex <=0) {
      info += "Please Select Available <br>";
      isperfect = false;
    } 
    ...
    if (copyrightlicc === "") {
      info += "Please enter Copyright Details<br>";
      isperfect = false;
    }
    ...
    $("#res5cms_form").submit(function (e) {
      e.preventDefault();
      $("#res5cms_form .form-control").removeClass("is-invalid");
     ...
     //Bootstrap error validation 
        var form_validate = $("#res5cms_form .form-control");
