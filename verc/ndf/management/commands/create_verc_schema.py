...
from ndf.models import DATA_TYPE_CHOICES
from ndf.models import node_collection, triple_collection
from ndf.models import Node, GSystemType, AttributeType, RelationType
from verc.settings import GSTUDIO_DEFAULT_SYSTEM_TYPES_LIST
...
class Command(BaseCommand):
  ...
  def handle(self, *args, **options):
    try:
      ...
      file_path = os.path.join(SCHEMA_ROOT, options['file_name'])
        ...
        if "csv" in file_extension:
          ...
          try:
            ...
            with open(csv_file_path, 'r', encoding="utf-8") as csv_file:
              ...
              #print("csv file:", json_file_content)
...
def parse_data_create_gtype(json_file_path):
  ...
  if "ST" in json_file_path:
    ...
    for json_document in json_documents_list:
      ...
      try:
        json_document['name'] = str(json_document['name'])
    ...
    for json_document in json_documents_list:
      # Process data in proper format
      try:
        print("inside try of parse data",json_document['object_scope'])
        json_document['name'] = str(json_document['name'])
        json_document['altnames'] = str(json_document['altnames'])
        json_document['help_text'] = str(json_document['help_text'])
        json_document["object_scope"] = []
        json_document["subject_scope"] = []
        json_document["attribute_type_scope"] = []
        '''
        if json_document["attribute_type_scope"]:
          json_document["attribute_type_scope"] = map(str,json_document["attribute_type_scope"])
        if json_document["subject_scope"]:
          json_document["subject_scope"] = map(str,json_document["subject_scope"])
        if json_document["object_scope"]:
          json_document["object_scope"] = map(str,json_document["object_scope"])
        print("scope values:",json_document["subject_scope"],json_document["object_scope"],json_document["attribute_type_scope"])
        '''
        ...
        print("bfr required", json_document['required'])
    ...
    print("in RTs processing")
    for json_document in json_documents_list:
      # Process data in proper format
      try:
        json_document['name'] = str(json_document['name'])
        json_document['inverse_name'] = str(json_document['inverse_name'])
        ...
        if json_document["relation_type_scope"]:
          json_document["relation_type_scope"] = json_document["relation_type_scope"]
        if json_document["subject_scope"]:
          json_document["subject_scope"] = []
        if json_document["object_scope"]:
          json_document["object_scope"] = []
...
def perform_eval_type(eval_field, json_document, type_to_create, type_convert_objectid):
  ...
  try:
    print ("eval data:",json_document[eval_field])
  ...
  for data in json_document[eval_field]:
    if (eval_field == "complex_data_type") and ((data in DATA_TYPE_CHOICES) or (json_document['data_type'] == "IS()")):
      ...
      def _append_to_type_list(eval_field, json_document, type_to_create, type_convert_objectid, data, inner_type_list):
        node = node_collection.find_one({'_cls': type_convert_objectid, 'name': data})
        #node = eval(type_convert_objectid).objects(name=eval(data))[0]
        print ("_appenddfdf",node)
        if node:
          if eval_field == "complex_data_type":
            inner_type_list.append(str(node['_id']))
...
def create_edit_type(type_name, json_document, user_id):
  ...
  """
  node = node_collection.find_one({'_cls': type_name, 'name': json_document['name']})
  if node is None:
    try:
      node = eval(type_name)()
      ...
      if user_id not in node.authors:
        node.authors.append(user_id)

      node.status = u"PUBLISHED"
      #print("befr save:",node. to_json())
      node.save()
