$(document).ready(function(){
    ...
    function processFilters(){
        ...
        let courseType_list = $("input[name='courseType']:checked").map(function() {return this.value;}).get().join(',');
        
        if($.trim(courseType_list.length) > 0) { 
            $("input[name='courseTypes']").val(courseType_list);
            url_str = "ctype="+courseType_list;
        };

        let courseCost_list = $("input[name='courseCost']:checked").map(function() {return this.value;}).get().join(',');
        
        if($.trim(courseCost_list.length) > 0) { 
            $("input[name='courseCosts']").val(courseCost_list);
            if($.trim(url_str.length)> 0)
            {
                url_str = url_str + "&";
            }
            url_str = url_str + "ccost="+courseCost_list;
        };

        let courseLocation_list = $("input[name='courseLocation']:checked").map(function() {return this.value;}).get().join(',');
        
        if($.trim(courseLocation_list.length) > 0) { 
            $("input[name='courseLocations']").val(courseLocation_list);
            if($.trim(url_str.length)> 0)
            {
                url_str = url_str + "&";
            }
            url_str = url_str + "clocation="+courseLocation_list;
        };
